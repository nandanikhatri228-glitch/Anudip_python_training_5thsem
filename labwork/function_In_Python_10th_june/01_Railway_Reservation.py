'''Statement 
A railway coach has seats represented as follows: 
seats = [ 
    "Booked", "Available", "Booked", "Booked", 
    "Available", "Available", "Booked", "Available", 
    "Booked", "Booked", "Available", "Booked" 
] 
Requirements 
Create the following functions: 
1.	count_seats(seats) 
Returns the number of booked and available seats. 
2.	first_available(seats) 
Returns the seat number of the first available seat. 
3.	occupancy_percentage(seats) 
Returns the percentage of occupied seats. 
4.	display_available_seats(seats) 
Displays all available seat numbers. 
'''
seats = [ 
    "Booked", "Available", "Booked", "Booked", 
    "Available", "Available", "Booked", "Available", 
    "Booked", "Booked", "Available", "Booked" 
] 

# ----------------------------------------------------------
# Function 1: Count booked and available seats
# ----------------------------------------------------------
def count_seats(seats):
    booked_seats = 0
    available_seats = 0
    for seat in seats:
        if seat == "Booked":
            booked_seats+=1
        else:
            available_seats+=1   
    return booked_seats, available_seats 
# ----------------------------------------------------------
# Function 2:Returns the seat number of the first available seat. 
# ----------------------------------------------------------
def first_availabe(seats):
    for seat in range(len(seats)):
        if seats[seat] =="Available":
            return seat+1
    return "No Available Seat" 
# ----------------------------------------------------------
# Function 3. Returns the percentage of occupied seats 
# ----------------------------------------------------------
def occupany_percentage(seats):
    occupied_seats = 0
    for seat in seats:
        if seat =="Booked":
            occupied_seats+=1
    percentage = (occupied_seats/len(seats))*100    
    return percentage      
# ----------------------------------------------------------
# Function 4: Display all available seat numbers
# ----------------------------------------------------------
def display_available_seats(seats):

    print("Available Seat Numbers:", end=" ")

    for index in range(len(seats)):

        if seats[index] == "Available":
            print(index + 1, end=" ")

    print()
# ==========================================================
# Main Program
# ==========================================================  
booked, available = count_seats(seats)
print("Booked Seats:",booked)
print("Available Seats:",available)
print("-------------------------------------------------")
print("First Available Seat:",first_availabe(seats))
print("--------------------------------------------------")
print("Occupied Percentage:", round(occupany_percentage(seats),2),"%")
print("---------------------------------------------------")
display_available_seats(seats)

'''Sample Output 
Booked Seats: 7 
Available Seats: 5 
 
First Available Seat: 2 
 
Occupancy Percentage: 58.33% 
 
Available Seat Numbers: 2 5 6 8 11 
'''